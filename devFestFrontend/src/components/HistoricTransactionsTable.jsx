...
import { getOrderBookBySymbol } from "../services";

const HistoricTransactionTable = ({ transactions, setData }) => {
  const [searchTerm, setSearchTerm] = useState("");

  useEffect(() => {
    if (searchTerm) {
      const delayFn = setTimeout(() => {
        let instrumentsFromDB = getOrderBookBySymbol(searchTerm);
        setData(instrumentsFromDB);
      }, 500);
